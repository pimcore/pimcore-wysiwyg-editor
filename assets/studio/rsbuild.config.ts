...
const buildRoot = path.resolve(__dirname, '..', '..', 'public', 'studio', 'build')
...
export default defineConfig({
  ...
  dev: {
    ...(isDevServer ? {} : { assetPrefix: '/bundles/pimcorewysiwygeditor/studio/build/' + buildId }),
    client: {
      host: 'localhost',
      port: 3033,
      protocol: 'ws'
  ...
  output: {
    manifest: true,
    assetPrefix: '/bundles/pimcorewysiwygeditor/studio/build/' + buildId,
    distPath: {
      root: buildPath
    }
  },
  tools: {
    bundlerChain: (chain) => {
      chain.output.uniqueName('pimcore_wysiwyg_editor_bundle')
    }
  },
  plugins: [
    ...
    pluginModuleFederation({
      name: 'pimcore_wysiwyg_editor_bundle',
